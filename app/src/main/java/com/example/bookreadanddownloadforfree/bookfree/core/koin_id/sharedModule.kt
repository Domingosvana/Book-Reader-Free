...
val sharedModule = module {
    ...
    single<RemoteAuthDataSource> { FirebaseRemoteAuthDataSource(get()) }
...
val sharedModule = module {
    ...
    // FirebaseAuth precisa ser declarado antes para ser injetado no DataSource
    single { FirebaseAuth.getInstance() }

    single<RemoteAuthDataSource> { FirebaseRemoteAuthDataSource(get()) }
    single<AuthRepository> { AuthRepositoryImpl(get()) }

    // --- Google Credential Manager Helper ---
    single { androidx.credentials.CredentialManager.create(androidContext()) }
    single { GoogleAuthUiClient(get() ) }

    // --- Livros (Data & Repository) ---
    singleOf(::KtorRemoteBookDataSource).bind<RemoteBookDataSource>()
    singleOf(::BookRepositoryImpI).bind<BookRepository>()

    // --- ViewModels ---

    // ViewModels Simples (Sem SavedStateHandle manual)
    viewModelOf(::BookListViewModel)
    viewModelOf(::SelectedBookViewModel)
    viewModelOf(::BookSearchViewModel)
    viewModelOf(::FilterViewModel)
    ...
    // ViewModels que usam SavedStateHandle
    viewModel { (handle: SavedStateHandle) ->
        BookDetailViewModel(
